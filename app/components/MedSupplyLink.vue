<template>
  <template v-if="showVariant">
    ...
    >
      Visit Our Medical Supply Website
    </BaseUiBreadCrumbCTA>
    ...
    <div v-if="chosen === 'under-image'" class="flex justify-center">
      ...
      >
        ...
        />
        <div>Visit Our Medical Supply Website</div>
      </BaseUiAction>
    </div>
  </template>
</template>
...
const MED_URL = 'https://goldengatemedicalsupply.com/';
...
const chosenRef = useABVariant('medical-supply', ['breadcrumb', 'under-image']);
...
function withUtm(baseUrl: string, variant: CtaVariant) {
  ...
  url.searchParams.set('utm_campaign', 'medical_supply_cta');
...
function trackCTA(variant: CtaVariant) {
  const params: Record<string, unknown> = {
    ...
    location: variant,
    event_category: 'CTA',
    event_label: `medical_supply_${variant}`,
  };
